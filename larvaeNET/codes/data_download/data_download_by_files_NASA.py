...
if(not os.path.isdir(f'{data_link}/images/')): os.makedirs(f'{data_link}/images/')
...
for pos in tqdm(range(0, len(data_keys))):
    ...
    for typo in types:
        ...
        if(info[typo] == [""]): continue
            ...
            if(typo == "FULL_BODY"):
                j = info[typo][name]
                f = open(f'{data_link}/images/{i}_{typo}_f.jpg','wb')
                f.write(request.urlopen(j).read())
                f.close()
            else:
                for (num, j) in enumerate(info[typo]):
                    f = open(f'{data_link}/images/{i}_{typo}_{num}.jpg','wb')
                    f.write(request.urlopen(j).read())
                    f.close()
